...
const Navbar = () => {
  ...
  const [modalState, setModalState] = useState({modal: false, phantom: false, metamask: false, connect: false});
  ...
  const handleModal = (modalID, state) => {
    setModalState({...modalState, [modalID]: state})
  }
  const handleWalletPath = (walletID) => {
    if(walletID == "metamask") {
      setWalletPath("./assets/svg/metamask.svg")
      setExtensionPath("https://chrome.google.com/webstore/detail/metamask/nkbihfbeogaeaoehlefnkodbefgpgknn")
    } else {
      setWalletPath("./assets/svg/phantom.svg")
      setExtensionPath("https://chrome.google.com/webstore/detail/phantom/bfnaelmomeimhlpmgjnjophhpkkoljpa")
    }
  }
  ...
  return (
    <div className="w-full flex lg:h-[68px] nav-h-52">
      ...
      {modalState.modal    && <Modal handleModal = {handleModal} handleWalletPath = {handleWalletPath} />} 
      {modalState.phantom  && <Phantom handleModal = {handleModal} />} 
      {modalState.metamask  && <Metamask handleModal = {handleModal} />} 
      {modalState.connect  && <ConnectModal handleModal = {handleModal} walletPath = {walletPath} extensionPath = {extensionPath}  />} 
      {open && (
        <div
